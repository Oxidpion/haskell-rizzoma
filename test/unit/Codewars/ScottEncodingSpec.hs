...
spec = do
  ...
  describe "The list type" $ do
    ...
    it "can be flattened in lists" $ do
        toList (concat (fromList [0..9]) (fromList [10..20])) `shouldBe` [0..20]
    it "can be predicated on its content" $ do
        null (fromList [0]) `shouldBe` False 
